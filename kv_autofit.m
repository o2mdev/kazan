...
% [modified_script, simy]  = kv_autofit(script, x, y, rangeidx)

% strFunc = 'f =n*x + b*a';
% cellVar = {'n = 1'; 'b = 2'; 'a = 5'};

% --------------------------------------------------------------------
function varargout = kv_autofit(scr_str, x, y, rangeidx)
funlist = {'kv_expdata';'polyfit';'kv3_lshape';'lorentzian';'gaussian';'lshape'; 'exp';'log';'cos';'sin';'abs'; './'; '.*';'.^'; ...
        '+'; '-'; '/'; '*'; '^'; ','; ''''; '='; '('; ')'};
...
otherwise
    ...
    elseif findstr(resstr, 'polyfit')
        ...
        strChi = ['sum((', resstr,' - y).^2)'];
        f = inline(strChi, 'in', 'x', 'y');
        
        opt = optimset('Display', 'on', 'MaxIter', MaxIter);
        
        for kk = 1:sz2
            in = fminsearch(f, in, opt, x(rangeidx), y(rangeidx, kk));
            eval(['simy(:,kk) = ', resstr, ';']);
        end
    end
