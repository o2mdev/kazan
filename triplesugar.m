% function [Freq, Spec] = triplesugar2D(Sys,Exp,Opt)
...
% Freq1 is equal to Freq2
%==========================================================================
function [Freqs, Spec] = triplesugar2D(Sy,Ex,Si)
%==========================================================================
...
for sp1=1:(snumber-1)
    for sp2=(sp1+1):snumber
        ...
        omega_l = Ex.Field/planck /1E9 * protongfactor * nmagn;
        ...
        H1zeeman(1,:) = -B(3,:);
        H1zeeman(2,:) = -B(1,:) + 1i*B(2,:);
        
        H2zeeman(1,:) = -B(3,:);
        H2zeeman(2,:) = -B(1,:) + 1i*B(2,:);
        
        H1zeeman = H1zeeman.*0.5;
        H2zeeman = H2zeeman.*0.5;
        
        H1hpf(1,:)= A1xyz(3,:);
        H1hpf(2,:)= A1xyz(1,:) - 1i* A1xyz(2,:);
        
        H2hpf(1,:)= A2xyz(3,:);
        H2hpf(2,:)= A2xyz(1,:) - 1i* A2xyz(2,:);
