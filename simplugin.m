...
% Last Modified by GUIDE v2.5 13-Apr-2009 13:55:25
...
if nargin == 0  % LAUNCH GUI
    ...
    outstr = inimanage([pathstr, '\kazan.ini']);
...
str2 = [strtrim(str1(3:end)), ' '];

if strcmp(strtrim(name(3:end)), 'f')
...
[name, str1] = strtok(Res{val}, '=');
name = strtrim(name); opt_mode=name(1:2); name=name(3:end);
commOK = 1;
...
case 'function'
    ...
    for k=1:size(Res,1)
        if ~strcmp(strtrim(strtok(Res{k},'=')),'f')
          try eval([Res{k}(3:end) ';']); catch; end
        end
    end
...
otherwise
    ...
    Res{val} = [opt_mode, name ' = ' strtrim(newval)];
end
...
str = get(handles.list, 'String');
for k=1:size(str,1),
    if strcmp(strtrim(strtok(str{k}(3:end),'=')),'f')
        func = str{k}(3:end);
    else
        try eval([str{k}(3:end) ';']); catch end
    end
end
try eval([func ';']); 
catch 
...
try
    ...
    MultFld1 = {'Exp','Opt', 'Exp','Sys', 'Shift', 'Exp', 'Exp', 'Sys', 'Exp'};
    MultFld2 = {'Field','ThetaRange', 'ExciteWidth','lwEndor', 'Scale', 'mwFreq', 'asym', 'lwEndor', 'TrFreq'};
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    MultSize = 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     
    ...
    for kk=1:MultSize
        ...
        if handles.is2D
          ...
          if safeget(handles, 'use_cache', 0)
            fit_struct = struct('Sys', Sys, 'Exp', Exp, 'Opt', Opt, 'Shift', Shift);
            res = GetFromCache(handles.cache_index_file, handles.cache_file, fit_struct);
            if isempty(res)
              [B,Sp] = feval(sim, Sys, Exp, Opt);
              Save2Cache(handles.cache_index_file, handles.cache_file, ...
                fit_struct, struct('Fld', B, 'Spec', Sp))
            else
              disp('Using Cache.');
              B = res.Fld; Sp = res.Spec;
            end
          else
            [B, Sp] = feval(sim, Sys, Exp, Opt);
          end
    ...
    if max(handles.sim{num}.amp)==0
        ...
        handles.sim{num}.y = Spec./handles.sim{num}.amp(ones(Exp.nPoints, 1), :); 
    end
    guidata(handles.figure1, handles);
    if nargout > 0, 
        varargout{1} = handles;
...
if (~errr)&(bsum) & max(y_sum)~=min(y_sum)
    ...
    hand.out{end}.y = y_sum+sh(ones(size(yTemp, 1), 1), :);
...
case {handles.mLoad, handles.ptChange}
    [fname,pname] = uigetfile([handles.currdir, '\*.m'],'Open Script');
    if fname == 0, return; end
    handles.currdir = pname;
    handles.sim{val}.ScriptName = [pname '\' fname];
...
case handles.mLoadAll
    if exist('uigetdir')==5
        ...
        [a, directory_name] = uiputfile([handles.currdir,'\select.dir'],'Select directory');
    end
    if ~ischar(directory_name)|isempty(directory_name), return; end
    handles.currdir = [directory_name, '\'];
    files = dir([handles.currdir,'*.m']);
    scripts = {files.name};
...
case {handles.mSave, handles.ptSave}
    ...
    [fname,pname] = uiputfile([handles.currdir, '\', string(3:end), '.m'],'Save Script');
    if fname == 0, return; end
    handles.currdir = pname;
...
case handles.mSaveAll
    ...
    for num = cnum
        ...
        [pa,name] = fileparts(string);
        SaveScript([directory_name,'\', name, '.m'], handles.sim{num}.Script);
...
inimanage([pathstr, '\kazan.ini'], outstr);
...
try
    [pathstr] = fileparts(get(handles.figure1, 'FileName'));
    outstr = inimanage([pathstr, '\kazan.ini']);
    outstr.Common_For_Plugins.currDir = handles.currdir;
...
[fname, pname] = uigetfile([handles.currdir, '\*.m'],title);
handles.currdir = pname;
if fname == 0, return; end
...
if handles.process, y = processing(y, ax);end
...
if hObject==handles.mAboutHelp
    [fpath] = fileparts(which('kazan'));
    filename = ['file:///',fpath,'\help\simplugin_sims.html'];
    filename(filename=='\')='/';
    stat = web(filename);
else
    msgbox({'simplugin';
        ...
        'Shift parameters:';
        'x and y - simulation shift along corresponding axis <0>';
...
for kk=1:length(Str)
    if isempty(strfind(Str{kk},par))
        ...
        if cur_ins<=max_insert
            Str1{end+1}=[stat,Str2{cur_ins}];
            cur_ins=cur_ins+1; 
        end
    end
end

for kk=cur_ins:max_insert, Str1{end+1}=['l-',Str2{kk}]; end
...
newval = strtrim(get(handles.eOpt, 'String'));
name = strtok(Res{val}, '=');
...
OptRng=[];
for kk=1:size(str,1)
    try eval([str{kk}(3:end) ';']); catch; end
end
...
try
    ...
    for kk=1:nsim
        ...
        MultFld1 = {'Exp','Opt', 'Exp','Sys', 'Shift'};
        MultFld2 = {'Field','ThetaRange', 'ExciteWidth','lwEndor', 'Scale'};
        MultSize = 1;
        % Determine multiple argument variables 
...
% % 140 210
try
  FigaPos = get(h, 'Position');

  if FigaPos(3)<130, FigaPos(3) = 130; set(h, 'Position', FigaPos); end
  if FigaPos(4)<210, FigaPos(4) = 210; set(h, 'Position', FigaPos); end
  brd = 5; % board
  butCalcH = 21; % points
  UpperFrameH = 25;
  EditH = 15;
  ...
  set(handles.butCalc,'Units', 'points', 'Position', Calc2Pos);

  CalcPos = [2*brd + butCalcH, brd, butCalcH, butCalcH]; % Calculate All

  set(handles.pbCalcAll, 'Units', 'points', 'Position', CalcPos);
  ReCalcPos = [3*brd + 2*butCalcH, brd, butCalcH, butCalcH]; % Calculate
  set(handles.pbReCalc,'Units', 'points', 'Position', ReCalcPos);

  %  Optimization
  BotFramePos = [brd, sum(Calc2Pos([2, 4]))+brd, FigaPos(3)-2*brd, 100];
  set(handles.frame2,'Units', 'points', 'Position', BotFramePos);

  SlUD_x= 17; SlUD_y = 20;
  Pop2Pos = [BotFramePos(1) + brd, BotFramePos(2)+brd+2, ...
    (BotFramePos(3) - 4*brd - SlUD_x)/2, 15];
  set(handles.pmOpt,'Units', 'points', 'Position', Pop2Pos);

  eResPos = [Pop2Pos(1) + brd + Pop2Pos(3), BotFramePos(2)+brd+2, ...
    (BotFramePos(3) - 4*brd - SlUD_x)/2, 15];
  set(handles.eOpt,'Units', 'points', 'Position', eResPos);

  SlPos2 = [eResPos(1) + eResPos(3) + brd, eResPos(2)-2, SlUD_x, SlUD_y];
  set(handles.slOpt,'Units', 'points', 'Position', SlPos2);

  lbOptPos = [Pop2Pos(1), sum(Pop2Pos([2, 4])) + brd, BotFramePos(3)-2*brd, BotFramePos(4) - 35-4*brd];
  set(handles.lbOpt,'Units', 'points', 'Position', lbOptPos);

  pmMethPos = [Pop2Pos(1), sum(lbOptPos([2, 4])) + brd, BotFramePos(3)-2*brd, 15];
  set(handles.pmOptMethod,'Units', 'points', 'Position', pmMethPos);

  textOptPos = [Pop2Pos(1) + brd, sum(BotFramePos([2, 4]))-brd, 50, 10];
  set(handles.text3,'Units', 'points', 'Position', textOptPos);
  ...
  SlPos = [EditPos(1), EditPos(2) + EditPos(4)+brd, 22, 18];
...
for cc = 1:num-1 % all simulations
    ...
    for ci = 1:length(refscr) % all strings in reference list
        ...
        for cj = 1:length(handles.sim{cc+1}.Script)  % all strings in current list
            ...
            if strcmp(r_str, c_str)&strcmp(strtrim(r_name(2:end)), strtrim(c_name(2:end)))&strcmp(strtrim(r_val(2:end)), strtrim(c_val(2:end)))
               counter(end+1) = ci;
            end
        end
    end
...
for cc = 1:num % all simulations
    ...
    for cj = 1:length(handles.sim{cc}.Script)  % all strings in current list
        ...
        [c_str, c_name] = strtok(c_names, '.');
        if strcmp(strtrim(r_str), strtrim(c_str))&strcmp(strtrim(r_name(2:end)), strtrim(c_name(2:end)))
            counter = cj;
            break;
        end
    end
...
% --------------------------------------------------------------------
function mEditCache_Callback(hObject, eventdata, handles)

status = get(handles.mEditCache, 'Checked');

if strcmp(status, 'off')
  [cache_index_file,cache_file,b] = KazanCacheGUI();
  if b
    handles.cache_index_file = cache_index_file;
    handles.cache_file = cache_file;
    handles.use_cache = 1;
    set(handles.mEditCache, 'Checked', 'on');
  end
else
    handles.use_cache = 0;
    set(handles.mEditCache, 'Checked', 'off');
end
