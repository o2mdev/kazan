...
if strcmp(Res{val}(1:3), 'N ='), 
    handles = lshapes(handles); 
    Res = handles.Script{1};
else
...
if handles.ShowLines
    strrr = {'SepLines_alt', 'SepLines'};
    eval(['axxx = ax.', lower(prj), ';']);
    yy = feval(strrr{handles.par.type+1}, axxx(:), handles.par);
...
try
    ...
    for i =1:length(handles.Script{num})
        if ~isempty(findstr(handles.Script{num}{i}, 'shapes')),
            isitlshapes = 1;
        end
    end
...
if findstr(str{val}, 'polyfit'), 
    ...
    if findstr(str{val}, 'shapes'), 
        %doppar = gen_par(str);
        if handles.par.type
            f = lshapes_func(x,handles.par); % don't fit, just calculate
        else
            f = altshapes_func(x,handles.par); % don't fit, just calculate
        end
    else
        eval([str{val}, ';']);
    end
...
maxiter = 100000;
largescale = 'off';
funcname = {'altshapes_func', 'lshapes_func'};
% for i = 1:length(script)
%     if strcmp(script{i}(1:3), 'f ='), 
%         
%         continue;end
%     eval([script{i}, ';']);
% end
doppar = gen_par(script);
% doppar.Nsubs = N;
% doppar.harm = harm;
...
funcid = doppar.type+1;
if doppar.fit_amps, 
    %linefit_fitamp(par, x, data, doppar)
    start_param = [doppar.x0, doppar.fwhh, doppar.amp];
    doppar.par = doppar;
    param = fminsearch(@linefit_fitamp, start_param, [], x, data, doppar, funcname{funcid});

        doppar.x0 = min(param(1:N) , doppar.par.x0*(1+doppar.dx0));
        doppar.x0 = max(doppar.x0, doppar.par.x0*(1-doppar.dx0));
    
        doppar.fwhh = min(param([1:N]+N), doppar.par.fwhh*(1+doppar.dfwhh));
        doppar.fwhh = max(doppar.fwhh,  doppar.par.fwhh*(1-doppar.dfwhh));
    
        doppar.amp = min(param([1:N]+2*N), doppar.par.amp*(1+doppar.damp));
        doppar.amp = max(  doppar.amp,   doppar.par.amp*(1-doppar.damp));
    
        y = feval(funcname{funcid}, x, doppar);
else
    start_param = [doppar.x0, doppar.fwhh];
    doppar.par = doppar;
    param = fminsearch(@linefit_noamp, start_param, [], x, data, doppar, funcname{funcid});
...
%--------------------------------------------------------------------
function f=linefit_noamp(par, x, data, doppar, funcname)
...
% doppar.fwhh = par([1:N]+N);
    doppar.x0 = min(par(1:N) , doppar.par.x0*(1+doppar.dx0));
    doppar.x0 = max(doppar.x0, doppar.par.x0*(1-doppar.dx0));
    
    doppar.fwhh = min(par([1:N]+N), doppar.par.fwhh*(1+doppar.dfwhh));
    doppar.fwhh = max(doppar.fwhh,  doppar.par.fwhh*(1-doppar.dfwhh));
    
y = feval(funcname, x, doppar);
%y=lshapes_func(x, doppar);
f = sum((data - y).^2);

%--------------------------------------------------------------------
function f=linefit_fitamp(par, x, data, doppar, funcname)
% par - struct: [xo[1], xo[2], fwhh[1], fwhh[2]]
% doppar - structure. doppar.amp, doppar.harm, etc.
% convention: all stuff here must be columns
% accept par!
...
% doppar.amp = par([1:N]+2*N);
    ...
    doppar.amp = min(par([1:N]+2*N), doppar.par.amp*(1+doppar.damp));
    doppar.amp = max(  doppar.amp,   doppar.par.amp*(1-doppar.damp));
%y=lshapes_func(x, doppar);
y = feval(funcname, x, doppar);
f = sum((data - y).^2);
...
for i = 1:N
    ...
    ty = lshape(x, xo, fwhh, doppar.harm, doppar.dist(i));
    if max(abs(ty)),
        y = ty/max(abs(ty))*amp+y;
    end
end
%--------------------------------------------------------------------
function y=altshapes_func(x, par)
% amplitudes are part of fitting parameters
N = par.Nsubs;
nn = length(x);
y = x(:)*0;
...
if doppar.type % lshapes
    out_script = {'f = lshapes(N)',...
            ...
            ['amp_range = ', num2str(doppar.damp)],...
            ['x0_range = ', num2str(doppar.dx0)],...
            ['fwhh_range = ', num2str(doppar.dfwhh)],...
        };
    ...
    for i = 1:doppar.Nsubs
        ...
        out_script{end+1} = ['x0(', num2str(i), ') = ', num2str(doppar.x0(i))];
        out_script{end+1} = ['fwhh(', num2str(i), ') = ', num2str(doppar.fwhh(i))];
        out_script{end+1} = ['c0(', num2str(i), ') = ', num2str(doppar.c0(i))];
        out_script{end+1} = ['c1(', num2str(i), ') = ', num2str(doppar.c1(i))];
        out_script{end+1} = ['c2(', num2str(i), ') = ', num2str(doppar.c2(i))];
    end
end
%--------------------------------------------------------------------
...
for i = 1:length(script)
    if strcmp(script{i}(1:3), 'f ='), 
        whichs = findstr(script{i}, 'lshapes');
        continue;
    end
    eval([script{i}, ';']);
end
...
if ~isempty(whichs)
    if isempty(distortion), distortion = x0*0; end
    if isempty(harm), harm = 1; end
    doppar.type = 1;
    doppar.Nsubs = N;
    doppar.harm = harm;
    doppar.damp = amp_range;
    doppar.dfwhh = fwhh_range;
    doppar.dx0 = x0_range;
    doppar.dist =  distortion;
    doppar.maxiter = maxiter;
    doppar.fit_amps = fit_amps;
    ...
    if isempty(c0), c0 = x0*0; end
    if isempty(c1), c1 = x0*0; end
    if isempty(c2), c2 = x0*0; end
    doppar.type = 0;
    doppar.Nsubs = N;
    doppar.maxiter = maxiter;
    doppar.fit_amps = fit_amps;
...
function handles = lshapes(handles)
...
if par.type % lshapes
    ...
    dist(1:len) = dist(1:len);
    par.dist = dist;
else % altshapes
    c0 = zeros(1, par.Nsubs);
    c1 = zeros(1, par.Nsubs);
    c2 = zeros(1, par.Nsubs);
    c0(1:len) = par.c0(1:len);
    c1(1:len) = par.c1(1:len);
    c2(1:len) = par.c2(1:len);
    par.c0 = c0;
    par.c1 = c1;
    par.c2 = c2;
end
...
for i = 1:N
    ...
    amp = par.amp(i);
    if fwhh<=0, fwhh=1e-16; end
    ty = lshape(x, xo, fwhh, par.harm, par.dist(i));
    if max(abs(ty)),
        yy(:, i) = ty/max(abs(ty))*amp+yy(:, i);
    end
end

%--------------------------------------------------------------------
function yy = SepLines_alt(x, par)
N = par.Nsubs;
nn = length(x);
yy = zeros(nn, N);
