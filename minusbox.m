...
% Last Modified by GUIDE v2.5 25-Aug-2005 12:41:30
...
if isref
    % adjust dimesiones
    [szRefx,szRefy] = size(tmpRef.y);
    [szSecx,szSecy] = size(tmpSec.y);
    
    if szRefy ~= szSecy
        if szRefy == 1
          tmpRef.y(:,1:szSecy)=  tmpRef.y(:,ones(szSecy,1));
          szRefy = szSecy;
          tmpRef.ax.y = tmpSec.ax.y;
        end
        if szSecy == 1
          tmpSec.y(:,1:szRefy)=  tmpSec.y(:,ones(szRefy,1));
          szSecy = szRefy;
          tmpSec.ax.y = tmpRef.ax.y;
        end
    end
...
guidata(h.hh, hand);
[path,name] = fileparts(get(h.hh, 'FileName'));
