...
% Last Modified by GUIDE v2.5 12-Feb-2016 21:42:17
...
for kk = 1:Num
    ...
    hand.out{kk}.ax = trace.Data.ax;
    hand.out{kk}.ax.x = trace.Data.ax.x*trace.ZoomX;
    switch h.ZoomUnits
    case 'Norm'
        zoom = trace.ZoomY;
    case 'dB'
    ...
    if safeget(trace, 'change2D', 0)
        sh = safeget(trace, 'ShiftY2D', 0);
        zm = safeget(trace, 'ZoomY2D', 1);
        hand.out{kk}.y = (trace.Data.y*zoom*trace.YSign - sh).*zm;
    else
        hand.out{kk}.y = trace.Data.y*zoom*trace.YSign; 
    end
    
    hand.out{kk}.ax.s = trace.ShiftY;
    hand.out{kk}.ax.dx= trace.ShiftX;
...
% --------------------------------------------------------------------
function mProcAddAll_Callback(hObject, eventdata, handles)
hand = guidata(handles.hh);
Num = length(handles.trace);

if Num < 2, return; end

[xx,yy]=preprocess(handles.trace{1}, handles.process, handles.ZoomUnits);
  
for kk = 2:Num
  [xx1,yy1]=preprocess(handles.trace{kk}, handles.process, handles.ZoomUnits);
  
  yy2 = interp1(xx1, yy1, xx);
  yy = yy + yy2;
end
...
hand.out{1}.ax.x = xx;
hand.out{1}.y= yy;
hand.out{1}.title = ['Trace', num2str(kk)];
...
function [xx,yy]=preprocess(trace, doprocess, ZoomUnits)
if doprocess, 
  [yy, ax] = processing(trace.Data.y, trace.Data.ax);
  xx = ax.x;
else
  yy = trace.Data.y;
  xx = trace.Data.ax.x;
...
if safeget(trace, 'change2D', 0)
  sh = safeget(trace, 'ShiftY2D', 0);
  zm = safeget(trace, 'ZoomY2D', 1);
  yy = (yy*zoom*trace.YSign - sh).*zm;
else
  yy = yy*zoom*trace.YSign;
end
