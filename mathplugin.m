...
% Last Modified by GUIDE v2.5 30-Oct-2006 10:11:48
...
% --------------------------------------------------------------------
function varargout = Exec_Callback(h, ~, handles, varargin)
switch h
    case {handles.tgAbs, handles.tbReal, handles.tbImag, handles.tbAsIs}
        btns = [handles.tgAbs, handles.tbReal, handles.tbImag, handles.tbAsIs];
        set(btns,'Value', 0)
        set(h,'Value', 1)
end
...
% ----------------------------------------------------
function sl1Dst_Callback(hObject, ~, handles)
shift = get(handles.sl1Dst, 'Value');
set(handles.sl1Dst, 'Value', 0);
handles = guidata(handles.figure1);
hand = guidata(handles.hh);
try
    CurVal = str2num(get(handles.ed1Dst, 'String'));
    Val = min(floor(CurVal + shift), size(hand.src.y, 1));
    if Val <1, Val = 1; end
...
set(handles.sl2Dst, 'Value',0);
handles = guidata(handles.figure1);
hand = guidata(handles.hh);
try
    CurVal = str2num(get(handles.ed2Dst, 'String'));
    Val = min(floor(CurVal + shift), size(hand.src.y, 2));
    if Val <1, Val = 1; end
...
% ----------------------------------------------------
function tgCutData_Callback(hObject, ~, handles)
guidata(handles.figure1, handles);
Plott(handles);
...
if get(handles.tgCutData, 'Value')
    str1 = ['min(',handles.ed1Dst.String,', end) : min(',handles.ed1Dend.String,', end)'];
    if size(hand.src.y, 2)>1
        if get(handles.cbExpression2Dim, 'Value')
            str2 = get(handles.eCut2Dim, 'String');
        else
            str2 = ['min(',handles.ed2Dst.String, ', end) : min(', handles.ed2Dend.String,', end)'];    
        end
    else
        str2 = ':';
    end
...
if get(handles.tgAbs, 'Value')
    tmp.y = abs(tmp.y);
elseif get(handles.tbReal, 'Value')
    tmp.y = real(tmp.y);
elseif get(handles.tbImag, 'Value')
    tmp.y = imag(tmp.y);
end
...
if get(handles.pbPhase, 'Value')
    ...
    phase1 = str2double(get(handles.ePhase1, 'String'));
    phase1piv = str2double(get(handles.ePhase1pivot, 'String'));
    phase1sh = str2double(get(handles.ePhase1sh, 'String'));
    for jj=1:sz2
        tmp.y(:,jj) = tmp.y(:,jj).*exp(-1i*...
...
% --------------------------------------------------------------------
function varargout = phase_Callback(h, ~, handles, varargin)
guidata(handles.figure1, handles);
switch h
...
% --------------------------------------------------------------------
function varargout = slPhase_Callback(h, ~, handles, varargin)
guidata(handles.figure1, handles);
shift =get(handles.slPhase, 'Value');
set(handles.slPhase, 'Value',0);
...
% --------------------------------------------------------------------
function varargout = ePhase1_Callback(h, ~, handles, varargin)
Plott(handles);

% --------------------------------------------------------------------
function varargout = slPhase1_Callback(h, ~, handles, varargin)
guidata(handles.figure1, handles);
shift =get(handles.slPhase1, 'Value');
set(handles.slPhase1, 'Value',0);
CurVal=str2num(get(handles.ePhase1, 'String'));
cc = safeget(handles, 'PhaseFactor',1.);
Val = max(min(CurVal + shift*cc, 180), -180);
set(handles.ePhase1, 'String', num2str(Val))
...
% --------------------------------------------------------------------
function varargout = ePhase1pivot_Callback(h, ~, handles, varargin)
Plott(handles);

% --------------------------------------------------------------------
function varargout = ePhase1sh_Callback(h, ~, handles, varargin)
Plott(handles);

% --------------------------------------------------------------------
function varargout = slPhase2_Callback(h, ~, handles, varargin)
shift =get(handles.slPhase2, 'Value');
set(handles.slPhase2, 'Value',0);
...
% --------------------------------------------------------------------
function eCut2Dim_Callback(hObject, ~, handles)


% --------------------------------------------------------------------
function cbExpression2Dim_Callback(hObject, ~, handles)
